...
MyGame = ig.Box2DGame.extend({
	...
	_parseContactUserData: function(userData){
		...
		if(userData != null){
			if(userData.type == ig.PLAYER_ENTITY){
				...
				//unknwon entity type
				ig.log("b2ContactListener: Detected unknwon collision with entity of type " + userData.type)
			}
		}
	...
	//I couldn't resist calling it this
	executeKillList: function(){
		while(this.killList.length > 0)
			this.killList.pop().kill()
	},

	kill: function(entity){
		this.killList.push(entity);
	},
	
	update: function() {
		//kill stuff first
		this.executeKillList()
